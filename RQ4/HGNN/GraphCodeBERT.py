import os
import torch
from transformers import RobertaTokenizer, RobertaModel  # GraphCodeBERT 也用 Roberta 接口
import pandas as pd
    ...
    for dataset in datasets:
        for type in types:
            try:
                ...
                output_file = f'../docs/{dataset}/{type}/{type}_graphcodebert_vectors.xlsx'
                ensure_dir(output_file)
                df.to_excel(output_file, index=False)

                print(f"GraphCodeBERT vectors have been written to {output_file}")
